...
mod tests {
    ...
    fn convert_dt_to_ntp_test() {
        ...
        assert_eq!(sample.timestamp_subsec_millis(), (test_val as u32) / u32::MAX * 1000 );
        ...
        test_val = convert_dt_to_ntp(sample);
        assert_eq!(sample.timestamp(), (test_val >> 32) as i64 - NTP_UNIX_OFFSET);
        assert_eq!(sample.timestamp_subsec_millis(), ((test_val as u32) as u64 * 1000u64 / u32::MAX as u64 ) as u32 + 1);
    }
