...
- **SRv6 Segment List**: Best-effort forwarding (RFC 9503 §4 + RFC 8754), opt-in via `--srv6-return-forwarding` on the Linux IPv6 nix backend. The reflector sets the sticky `IPV6_RTHDR` socket option before `sendmsg`; Linux accepts this interface but rejects type 4 in the per-packet ancillary parser. Its exclusive send owner clears the setting before ordinary replies. The complete path preserves all requested SIDs and appends the UDP destination when needed, because Linux replaces Segment List[0] with that destination. A one-shot sticky-option probe gates the attempt; unavailable or failed insertion uses a normal reply with the Return Path **U-flag** set. Pnet retains that fallback. A strict three-namespace test now verifies real transit forwarding, open/authenticated traffic, CoS and subsequent ordinary replies; physical SRv6 fabrics remain separate verification.
...
- Every copy uses the same transport policy. On Linux `sendmsg` carries CoS and the matched source address; supported SRH uses the sticky option managed by the same exclusive send owner. Other supported platforms update cached CoS before the sole send owner's syscall when the setting changes. SRH/source/alternate-address failures take the shared best-effort fallback path; flag changes are signed with the request's selected key. Socket queue pressure stops the remaining burst and records a drop without downgrading metadata.
