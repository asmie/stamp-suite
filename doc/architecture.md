...
- Honours the requested reply-packet length up to the effective reply-size cap by appending an Extra Padding TLV (Type 1) before the HMAC TLV. When the request exceeds the cap, the C flag is set; the reply still pads to the cap. The cap is the smaller of `--reflected-control-max-size` (default 1500) and, on Linux with a non-wildcard `--local-addr`, the egress interface's MTU less the IP and UDP headers (`SIOCGIFMTU`, read once at startup — so 1472 on a 1500-byte IPv4 link). The reflector cannot use the sender's `getsockopt(IP_MTU)` route lookup, which only answers on a connected socket. Best-effort: a wildcard bind or a failed query leaves the flag as the only cap.
